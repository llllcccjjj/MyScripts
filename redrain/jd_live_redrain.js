...
//若获取不到activityId，更换livedetail参数
let livedetail={
  "liveId":"3466834",
  "url":"https://api.m.jd.com/client.action?functionId=liveActivityV842&clientVersion=9.3.6&build=86560&client=android&d_brand=Xiaomi&d_model=MI8&osVersion=10&screen=2115*1080&partner=xiaomi001&aid=3524914bc77506b1&oaid=274aeb3d01b03a22&openudid=3524914bc77506b1&eid=eidAef9a8121f9s8dlmkPL9TRL6I3dSjuDRbDHZZparHzRuMJ+ojIXbMperpUF0QK+gPsjBjXLYUU9FtgHGBv7+bZHeUkYnKC9eXT1Hf5cp2ILYAY+tl&sdkVersion=29&lang=zh_CN&uuid=3524914bc77506b1&area=17_1432_45217_45228&networkType=wifi&wifiBssid=unknown&uts=0f31TVRjBSuhwYLyvify4e6X%2BpqH6zgzI7TK1oWZi%2FXCt9HS%2F2ApOVqV%2FsCQW3559w2aSPHNVG3JXlbAy%2FaOvoaRcnsgc32wNQT9iQm9LeiZ4ezhF56vJh7ST4jBr1gXlHZG4xlDKvqgC4BwEC%2FdpG91ysPr3LHlMjRn%2FKWC7%2BbvFvtSWwDcWBxOeeeUOyEF14s%2B70yV29xbRvGq4FeKdA%3D%3D&st=1612874837309&sign=882090f19335e1a0d5364c4931e3f5fe&sv=121",
}

let ids = {
  '19': 'RRA4MfZhULNgQpEJmYGBNMBQZLMJ2am',
  '20': 'RRA4Rx79XHjGyU1vK1wWz4u1vT5aEDL',
...
!(async () => {
  ...
  console.log(`\n现在整点时间：`+hour)
  if(!$.activityId){
    $.log(`在线红包雨配置获取错误，读取内置activityId`)
    if (ids[hour]){
      $.activityId = ids[hour]
      $.log(`内置activityId获取成功`)
    } else{
      $.log(`无法获取内置activityId，请检查运行时间`)
      return
    }
  }else{
      $.log(`在线红包雨配置获取成功\n`)
...
function getRedRain() {
  ...
  return new Promise(resolve => {
    $.post(taskPostUrl(body), (err, resp, data) => {
      try {
        if (err) {
...
function taskPostUrl(body = body) {
  return {
    url: livedetail.url,
    body: body,
    headers: {
